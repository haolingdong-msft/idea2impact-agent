...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("OPENAI_API_KEY") or os.getenv("EDITPPT_BEARER_TOKEN"):
        auth_mode = (
            "EDITPPT_BEARER_TOKEN"
            if os.getenv("EDITPPT_BEARER_TOKEN")
            else "OPENAI_API_KEY"
        )
        print(f"{auth_mode} is set. API target: {_api_target_label()}.", file=sys.stderr)
        return
    if dry_run:
...
def _create_client():
    ...
    bearer_token = os.getenv("EDITPPT_BEARER_TOKEN")
    api_version = os.getenv("AZURE_OPENAI_API_VERSION")
    return OpenAI(
        api_key=os.getenv("OPENAI_API_KEY") or bearer_token,
